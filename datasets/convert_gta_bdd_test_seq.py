...
def _format_data(sess, image_reader, idx, tmp_dir, pathlist_A, pathlist_B, exemplar_img_name=None):
    ...
    if exemplar_img_name is None:
        path_B = _img_resize_flip(pathlist_B[idx], IMG_FLIP, tmp_dir, image_width=_IMG_WEIGHT, image_height=_IMG_HEIGHT)
    else:
        path_B = _img_resize_flip(os.path.join(os.path.dirname(pathlist_B[idx]),exemplar_img_name), IMG_FLIP, tmp_dir, image_width=_IMG_WEIGHT, image_height=_IMG_HEIGHT)
...
def _convert_dataset_one_pair_rec(dataset_dir, split_name, save_sub_dir, tmp_dir, tf_record_pair_num=np.inf, Shuffle=False, exemplar_img_name=None):
    ...
    assert split_name in ['test_seq'] #  'val'
            ...
            for shard_id in range(num_shards):
                ...
                with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
                    ...
                    for idx in range(start_ndx, end_ndx):
                        ...
                        example = _format_data(sess, image_reader, idx, tmp_dir, pathlist_A, pathlist_B, exemplar_img_name=exemplar_img_name)
...
if __name__ == '__main__':
    split_name = 'test_seq'
    dataset_dir = '/esat/dragon/liqianma/datasets/Adaptation/SG-GAN_data/gta_bdd_test_seq/'
    ...
    #################
    save_sub_dir = 'gta_bdd_{}_{}x{}'.format(split_name, _IMG_HEIGHT, _IMG_WEIGHT)
    ...
    _convert_dataset_one_pair_rec(dataset_dir, split_name, save_sub_dir, tmp_dir,
                                    tf_record_pair_num=100, Shuffle=True)



    #################
    # # exemplar_img_name = 'd6346aa2-083f81fc.jpg'
    # exemplar_img_name = 'e5d3f007-8ca16edb.jpg'
    # save_sub_dir = 'gta_bdd_{}_{}x{}_{}'.format(split_name, _IMG_HEIGHT, _IMG_WEIGHT, exemplar_img_name.split('.')[0])
    # if not os.path.exists(os.path.join(dataset_dir,save_sub_dir)):
    #     os.makedirs(os.path.join(dataset_dir,save_sub_dir))

    # _convert_dataset_one_pair_rec(dataset_dir, split_name, save_sub_dir, tmp_dir,
    #                                 tf_record_pair_num=np.inf, Shuffle=False, exemplar_img_name=exemplar_img_name)