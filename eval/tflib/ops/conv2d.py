...
def Conv2D_reflect(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1.):
    """
    inputs: tensor of shape (batch size, num channels, height, width)
    mask_type: one of None, 'a', 'b'
    ...
    with tf.name_scope(name) as scope:
        ...
        # inputs = tf.pad(inputs, [[0, 0], [0, 0], [1, 1], [1, 1]], mode="CONSTANT") ## for NCHW
        inputs = tf.pad(inputs, [[0, 0], [0, 0], [1, 1], [1, 1]], mode="REFLECT") ## for NCHW
        result = tf.nn.conv2d(
            input=inputs, 
            filter=filters, 
            strides=[1, 1, stride, stride],
